...
// Add this function to check working hours
...
// Add this function for the non-working hours notification
function showNonWorkingHoursModal() {
    const modal = document.getElementById('nonWorkingHoursModal');
    if (!modal.classList.contains('show')) {
        const now = new Date();
        const timeStr = now.toLocaleTimeString('ja-JP', {
            hour: '2-digit',
        ...
        document.getElementById('non-working-timestamp').textContent = timeStr;
        modal.classList.add('show');
    }
}
