...
function showPressureAlarm() {
    ...
    if (modal) {
        ...
        // Add notification (throttle to avoid duplicate notifications - max 1 per 5 seconds)
        const now = Date.now();
        if (now - lastAlarmNotificationTime > 5000) {
            addNotification('alarm', '警告: 空気圧低下', `F: ${frontPressure.toFixed(3)} MPa, R: ${rearPressure.toFixed(3)} MPa`);
...
function showIdleNotification() {
    ...
    if (modal) {
        ...
        // Add notification (throttle to avoid duplicate notifications - max 1 per 5 seconds)
        const now = Date.now();
        if (now - lastIdleNotificationTime > 5000) {
            addNotification('idle', '通知: システム待機中', `F: ${frontPressure.toFixed(3)} MPa, R: ${rearPressure.toFixed(3)} MPa`);
...
document.addEventListener('DOMContentLoaded', async () => {
    // Only load initial data if charts exist (dashboard page)
    if (document.getElementById('front-pressure-chart')) {
    ...
    loadNotificationsFromLocalStorage(); // Restore notifications from previous session
    updateDateTimeDisplay();
    setInterval(updateDateTimeDisplay, 1000);
});

// Listen for storage changes across tabs/pages (for notification sync)
window.addEventListener('storage', (event) => {
    if (event.key === 'dashboardNotifications' && event.newValue) {
        loadNotificationsFromLocalStorage();
    }
});

// Save chart data and notifications before page unload
...
window.addEventListener('click', (event) => {
  if (!event.target.matches('.notification-icon') && !event.target.closest('.notification-menu')) {
