...
function createCharts() {
    const chartOptions = (label, color, bgColor) => ({
        type: 'line',
        data: {
    ...
    frontPressureChart = new Chart(frontCtx, chartOptions('Front (MPa)', 'rgb(75, 192, 192)', 'rgba(75, 192, 192, 0.2)'));

    const rearCtx = document.getElementById('rear-pressure-chart').getContext('2d');
    rearPressureChart = new Chart(rearCtx, chartOptions('Rear (MPa)', 'rgb(255, 99, 132)', 'rgba(255, 99, 132, 0.2)'));
}
...
document.addEventListener('DOMContentLoaded', () => {
    createCharts(); // Initialize canvases first
    ...
    setInterval(updateAverages, 10000); // 10s is plenty for averages
});
