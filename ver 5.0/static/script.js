...
async function updateRealtimeData() {
    try {
        ...
        if (data.front_pressure !== undefined && data.rear_pressure !== undefined) {
            document.getElementById('front-pressure-value').innerText = data.front_pressure.toFixed(3);
            document.getElementById('rear-pressure-value').innerText = data.rear_pressure.toFixed(3);
            
            // Check pressure threshold and show alarm if needed
            if (data.front_pressure < LOW_PRESSURE_THRESHOLD || data.rear_pressure < LOW_PRESSURE_THRESHOLD) {
                showPressureAlarm();
            } else {
                hidePressureAlarm();
            }
